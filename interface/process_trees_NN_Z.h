...
bins_NN({8,200,1000}), // FIXME This changed for making control plots for Jan's comments May 14th
//bins_NN(bins),
...
//bins_NN_bkg(bins),
bins_NN_bkg({1,0,1000}), // FIXME just for the test
//bins_NN_bkg(bins),
//bins_NN_bkg({20,0,1}),
bins_FAKE({10,0,300}),
...
void HistTool::fillQCD_OS_CR(int cat, string name, double var1,  double weight) {
    if (name == "Data") {
        ...
        fakes_1d_OS_CR_Up.at(cat)->Fill(var1, -1*weight*0.8);
        fakes_1d_OS_CR_Down.at(cat)->Fill(var1, -1*weight*1.2);
    }
}
// This is CR to extract OS/SS ratio
void HistTool::fillQCD_SS_CR(int cat, string name, double var1,  double weight) {
    if (name == "Data") {
        ...
        fakes_1d_SS_CR_Up.at(cat)->Fill(var1, -1*weight*0.8);
        fakes_1d_SS_CR_Down.at(cat)->Fill(var1, -1*weight*1.2);
    }
}
