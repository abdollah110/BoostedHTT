...
//            os_ss_values.push_back(OS_SS_vbf);
        ...
        void HistTool::writeTemplates() {
            ...
            for (auto cat : hists_1d) {
                ...
                //protection for Madgraph
                // ADD protection
//                for (int i = 0 ; i < hist->GetNbinsX(); i++){
//                        if (hist->GetBinContent(i+1) <0 ){
//                            float negBin=hist->GetBinContent(i+1);
//                            float totIntegral =hist->Integral();
//                            hist->SetBinContent(i+1, 0.001);
//                            std::cout<< cat.first.c_str() << "   Madgraph bin of "<<i << "  ratio bin/TotIntegral"<< negBin/totIntegral <<"\n";
//                    }
//                }
...
//                }
                ...
                fake_hist_shape_Up->SetName("QCD_Up");
                fake_hist_shape_Down->SetName("QCD_Down");
...
//        float D0_binning[5]={0,0.35,0.5,0.65,1.0}; // for ggH AC coupling
//        float D0_binning[5]={0,0.2,0.4,0.8,1.0}; // for HVV AC coupling
        // basically a map from 2 inputs -> 1 Category
        
//        Categories HistTool::getCategory(std::vector<Float_t> D0_binning, double vbf_var3, double vbf_var4 = -1) {
////            double edge = 1./6.;
//            if (vbf_var3 > 0 && vbf_var3 <= D0_binning[1]) {
//                return vbf_ggHMELA_bin1;
//            } else if (vbf_var3 <= D0_binning[2]) {
//                return vbf_ggHMELA_bin2;
//            } else if (vbf_var3 <= D0_binning[3]) {
//                return vbf_ggHMELA_bin3;
//            } else if (vbf_var3 <= D0_binning[4]) {
