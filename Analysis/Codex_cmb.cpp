...
int main() {
    ...
    bkg_procs["mj_cmb"] = {"ZTT", "W", "QCD", "TT","VV","SingleTop"};
 ...
 // Or equivalently, specify the mass points explicitly:
    ...
    for (string era : {"13TeV"}) {
        for (string chn : chns) {
            ...
            string file = aux_shapes + input_folders[chn] + "/_mj_LQMass_Final_cmb.root";
    ...
    .AddSyst(cb, "CMS_lumi_$ERA", "lnN", SystMap<era>::init
//             ({"7TeV"}, 1.022)
             ({"13TeV"}, 1.024));
    //! [part5]
...
//    .AddSyst(cb, "CMS_scale_met_JES", "shape", SystMap<>::init(1.00));
    ...
    cb.cp().backgrounds().ExtractShapes(
                                        aux_shapes + "/_mj_LQMass_Final_cmb.root",
                                        "$BIN/$PROCESS",
...
//    bbb.AddBinByBin(cb.cp().backgrounds(), cb);
    ...
    string folder = "outputCodex_cmb_PostCWR_v4/LIMITS";
