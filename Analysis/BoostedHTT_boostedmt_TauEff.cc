...
int main(int argc, char* argv[]) {
    ...
    outTr->Branch("isGenMu_",&isGenMu_,"isGenMu_/O");
    ...
    for (Int_t i = 0; i < nentries_wtn; i++) {
        ...
        TLorentzVector Mu4Momentum,SubTau4Momentum, Z4Momentum, Met4Momentum, Mu4MomentumNominal, SubTau4MomentumNominal;
        //=========================================================================================================
        // Lead tau selection
        ...
        bool isGenMu= isMatchedToGenMu(Mu4Momentum);
        ...
        SubTau4Momentum.SetPtEtaPhiM(boostedTauPt->at(idx_tau),boostedTauEta->at(idx_tau),boostedTauPhi->at(idx_tau),boostedTauMass->at(idx_tau));
        SubTau4MomentumNominal = SubTau4Momentum;
        bool isGenTauSub= isMatchedToGenTau(SubTau4Momentum);

        if (SubTau4Momentum.Pt() <= 30 || fabs(boostedTauEta->at(idx_tau)) >= 2.3 ) continue;
...
//        if (boostedTauByLooseMuonRejection3->at(idx_tau) < 0.5) continue;
        ...
        dR_lep_lep= SubTau4Momentum.DeltaR(Mu4Momentum);
        if (nJet < 1 ) continue;
        TLorentzVector LeadJet= getLeadJet(Mu4Momentum, SubTau4Momentum,JetSys);
            
        //=========================================================================================================
        // Event Selection
        //=========================================================================================================
        Met4Momentum.SetPtEtaPhiM(Met, 0, Metphi, 0);
        Z4Momentum=SubTau4Momentum+Mu4Momentum;
        TLorentzVector higgs = SubTau4Momentum+Mu4Momentum +Met4Momentum;
...
//        if (higgs_pT < 250 ) continue; //FIXME removed for Validation study
        ...
        OS = muCharge->at(lepIndex) * boostedTauCharge->at(idx_tau) < 0;
        SS =  muCharge->at(lepIndex) * boostedTauCharge->at(idx_tau) > 0;
//        lep1IsoPassL = boostedTauByLooseIsolationMVArun2v1DBoldDMwLTNew->at(tauIndex) > 0.5;
        tauIsoPassL = boostedTauByLooseIsolationMVArun2v1DBoldDMwLTNew->at(idx_tau) > 0.5;
//        lep1IsoPassV = boostedTauByVLooseIsolationMVArun2v1DBoldDMwLTNew->at(tauIndex) > 0.5;
        tauIsoPassV = boostedTauByVLooseIsolationMVArun2v1DBoldDMwLTNew->at(idx_tau) > 0.5;


        if (SS) continue;
        if (!isGenTauSub || !isGenMu) continue;
