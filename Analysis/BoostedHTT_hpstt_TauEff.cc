...
int main(int argc, char* argv[]) {
    ...
    for (Int_t i = 0; i < nentries_wtn; i++) {
        ...
        LeadTau4Momentum.SetPtEtaPhiM(tauPt->at(idx_leadtau),tauEta->at(idx_leadtau),tauPhi->at(idx_leadtau),tauMass->at(idx_leadtau));
        LeadTau4MomentumNominal = LeadTau4Momentum;
        bool isGenTauLead= isMatchedToGenTau(LeadTau4Momentum);
        
        if (LeadTau4Momentum.Pt() <= 30 || fabs(tauEta->at(idx_leadtau)) >= 2.3 ) continue;
        if (taupfTausDiscriminationByDecayModeFinding->at(idx_leadtau) < 0.5 ) continue;
//        if (tauByIsolationMVArun2v1DBoldDMwLTraw->at(idx_leadtau) < -0.5) continue;
//        if (tauByMVA6VLooseElectronRejection->at(idx_leadtau) < 0.5) continue;
//        if (tauByLooseMuonRejection3->at(idx_leadtau) < 0.5) continue;
        plotFill("cutFlowTable",3 ,15,0,15);
        
        //=========================================================================================================
        // sublead Tau selection
        int idx_subleadtau= subtauIndex;
        SubTau4Momentum.SetPtEtaPhiM(tauPt->at(idx_subleadtau),tauEta->at(idx_subleadtau),tauPhi->at(idx_subleadtau),tauMass->at(idx_subleadtau));
        SubTau4MomentumNominal = SubTau4Momentum;
        bool isGenTauSub= isMatchedToGenTau(SubTau4Momentum);

        if (SubTau4Momentum.Pt() <= 30 || fabs(tauEta->at(idx_subleadtau)) >= 2.3 ) continue;
        if (taupfTausDiscriminationByDecayModeFinding->at(idx_subleadtau) < 0.5 ) continue;
//        if (tauByIsolationMVArun2v1DBoldDMwLTraw->at(idx_subleadtau) < -0.5) continue;
//        if (tauByMVA6VLooseElectronRejection->at(idx_subleadtau) < 0.5) continue;
//        if (tauByLooseMuonRejection3->at(idx_subleadtau) < 0.5) continue;
...
//        if (higgs_pT < 250 ) continue; //FIXME removed for Validation study
        ...
        OS = tauCharge->at(idx_leadtau) * tauCharge->at(idx_subleadtau) < 0;
        SS =  tauCharge->at(idx_leadtau) * tauCharge->at(idx_subleadtau) > 0;
        lep1IsoPassL = tauByLooseIsolationMVArun2v1DBoldDMwLT->at(idx_leadtau) > 0.5;
        lep2IsoPassL = tauByLooseIsolationMVArun2v1DBoldDMwLT->at(idx_subleadtau) > 0.5;
        lep1IsoPassV = tauByVLooseIsolationMVArun2v1DBoldDMwLT->at(idx_leadtau) > 0.5;
        lep2IsoPassV = tauByVLooseIsolationMVArun2v1DBoldDMwLT->at(idx_subleadtau) > 0.5;
        ...
        tauRawIso=tauByIsolationMVArun2v1DBoldDMwLTraw->at(idx_subleadtau);
        m_sv_=m_sv;
        //  Weights
