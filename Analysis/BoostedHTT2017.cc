...
//
            ...
            TLorentzVector Mu4Momentum, Jet4Momentum,KJet4Momentum,BoostTau4Momentum,Mu24Momentum;
            
            for  (int ib=0 ; ib < nBoostedTau; ib++){
                
                BoostTau4Momentum.SetPtEtaPhiM(boostedTauPt->at(ib),boostedTauEta->at(ib),boostedTauPhi->at(ib),boostedTauMass->at(ib));
                
//                if (!onLeadTau&&  boostedTauDecayMode->at(ib) > 0 && boostedTauByLooseIsolationMVArun2v1DBoldDMwLT->at(ib) > 0){
                    if (!onLeadTau&&  boostedTauDecayMode->at(ib) > 0){
                    leadingBoostedTauPt =boostedTauPt->at(ib);
                    onLeadTau= true;
//                }
                
                
                cout<< " nBoostedTau= "<<nBoostedTau <<"   ib "<< ib << "    \t "<<boostedTauPt->at(ib)<< " " <<boostedTauEta->at(ib)<< " " <<boostedTauPhi->at(ib)<< " " <<boostedTauMass->at(ib)<< " \n";
                
            
            
                        if (boostedTauPt->at(ib) > 500 ){
            
            for  (int imu=0 ; imu < nMu; imu++){
                Mu4Momentum.SetPtEtaPhiM(muPt->at(imu),muEta->at(imu),muPhi->at(imu),MuMass);
                
                if (BoostTau4Momentum.DeltaR(Mu4Momentum) < 0.8) cout<<"\t muon pT = "<<muPt->at(imu)<<"\n";
//
...
//                //                cout <<"\n\n\n";
                ...
                for (int ijet= 0 ; ijet < nJet ; ijet++){
                    ...
                    bool goodJet =  jetPt->at(ijet) > JetPtCut && fabs(jetEta->at(ijet)) < 3 && Jet4Momentum.DeltaR(BoostTau4Momentum) > 0.5;
                    if (! goodJet) continue;
...
//                        plotFill(CHL+"_LQMass",100,200,0,2000,1);
                    
                    
//                    plotFill(CHL+"_boostedTauMass",boostedTauMass->at(ib),200,0,2000,1);
