...
//            for (int i =0; i < 3 ;i++) {
                ...
                if (NN_out_vec[1] < 0 )continue;
                
                if (OS != 0  && lep1IsoPass && lep2IsoPass) {
                    hists_1d.at(categories.at(1)).back()->Fill(vbf_var1,  weight);
...
//                    fillQCD_Norm(i, name, NN_out_vec[i],  weight, meausred_OSSS);
                    fillQCD_Norm(1, name, vbf_var1,  weight, meausred_OSSS);
                }
                // qcd shape
                if (SS != 0 && lep1IsoPass && lep2IsoPass ){
//                    fillQCD_Shape(i, name, NN_out_vec[i],  weight, meausred_OSSS);
                    fillQCD_Shape(1, name, vbf_var1,  weight, meausred_OSSS);
                }
//            }
            NN_out_vec.clear();
