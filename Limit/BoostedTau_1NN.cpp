...
//    VString chns = { "mt_qcd","mt_signal","mt_ztt","et_qcd","et_signal","et_ztt","em_qcd","em_signal","em_ztt","tt_qcd","tt_signal","tt_ztt"};
    VString chns = { "mt_signal","et_signal","em_signal","tt_signal"};
...
//    bkg_procs["em_ztt"] = {"QCD", "TT","VV","ZTT","W"};
    ...
    VString mt_cat = {"mt_signal"};
    VString et_cat = {"et_signal"};
    VString tt_cat = {"tt_signal"};
    VString em_cat = {"em_signal"};
...
//        .AddSyst(cb, "CMS_scale_t_1prong1pizero_ztt"+year, "shape", SystMap<>::init(1.00));
//
//    cb.cp().process(ch::JoinStr({sig_procs, {"W", "TT","VV","ZTT"}})).channel(ch::JoinStr({ztt_cat_tau}))
//        .AddSyst(cb, "CMS_scale_t_1prong_ztt"+year, "shape", SystMap<>::init(1.00));
//
//    cb.cp().process(ch::JoinStr({sig_procs, {"W", "TT","VV","ZTT"}})).channel(ch::JoinStr({ztt_cat_tau}))
//        .AddSyst(cb, "CMS_scale_t_3prong_ztt"+year, "shape", SystMap<>::init(1.00));
//
//    cb.cp().process(ch::JoinStr({sig_procs, {"W", "TT","VV","ZTT"}})).channel(ch::JoinStr({qcd_cat_tau}))
//        .AddSyst(cb, "CMS_scale_t_1prong1pizero_qcd"+year, "shape", SystMap<>::init(1.00));
//
//    cb.cp().process(ch::JoinStr({sig_procs, {"W", "TT","VV","ZTT"}})).channel(ch::JoinStr({qcd_cat_tau}))
//        .AddSyst(cb, "CMS_scale_t_1prong_qcd"+year, "shape", SystMap<>::init(1.00));
//
//    cb.cp().process(ch::JoinStr({sig_procs, {"W", "TT","VV","ZTT"}})).channel(ch::JoinStr({qcd_cat_tau}))
//        .AddSyst(cb, "CMS_scale_t_3prong_qcd"+year, "shape", SystMap<>::init(1.00));
//
//    cb.cp().process(ch::JoinStr({sig_procs, {"W", "TT","VV","ZTT"}})).channel(ch::JoinStr({sig_cat_tau}))
//        .AddSyst(cb, "CMS_scale_t_1prong1pizero_sig"+year, "shape", SystMap<>::init(1.00));
//
//    cb.cp().process(ch::JoinStr({sig_procs, {"W", "TT","VV","ZTT"}})).channel(ch::JoinStr({sig_cat_tau}))
//        .AddSyst(cb, "CMS_scale_t_1prong_sig"+year, "shape", SystMap<>::init(1.00));
//
//    cb.cp().process(ch::JoinStr({sig_procs, {"W", "TT","VV","ZTT"}})).channel(ch::JoinStr({sig_cat_tau}))
//        .AddSyst(cb, "CMS_scale_t_3prong_sig"+year, "shape", SystMap<>::init(1.00));
//
//
//
