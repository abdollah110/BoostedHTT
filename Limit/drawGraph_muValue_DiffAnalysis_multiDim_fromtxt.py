...
TypesOfChannel=['2016','2017','2018','em','et','mt','tt','all']
Bins=['r_H_PTH_0_350','r_H_PTH_350_450','r_H_PTH_450_600','r_H_PTH_GT600']

if '0p5' in inputDir:
    
    TypesOfChannel=['2016','2017','2018','et','mt','tt','all']
    Bins=['r_H_PTH_450_600','r_H_PTH_GT600']
    
    
n = len(Bins)
m = len(Bins)
    
for ch in TypesOfChannel:
    y,dy,dyl= array( 'd' ), array( 'd' ), array( 'd' )
    for bin in Bins:
        print '\n\n %%%%%%%%%%%%%% stating {} {} '.format(ch,bin)
#        if bin == 'bin0':

        with open('{}/higgsCombineTest.MultiDimFit.mH125_{}_{}.root_scan.txt'.format(inputDir,ch,bin),'r') as f:
            lines=f.readline()
...
#
#        input1=TFile('FITALL/higgsCombineTest.FitDiagnostics.mH125_{}_{}.root'.format(ch,bin),'r')
...
#        else:
#
#            input1=TFile('{}/{}_{}_{}/125/higgsCombineTest.FitDiagnostics.mH125.root'.format(infile, ch,wp,bin),'r')
#            tr1=input1.Get('limit')
#            for i in tr1:
#                if round(i.quantileExpected,2) == 0.5:
#                    y.append(round(i.limit,2))
...
#    print 'm is ', m
    ...
    gr_tot.SetTitle( '{}'.format(ch) )
    gr_tot.GetXaxis().SetTitle( 'Higgs p_{T} [GeV]' )
    if 'jet' in inputDir or 'Jet' in inputDir: gr_tot.GetXaxis().SetTitle( 'Leading Jet p_{T} [GeV]' )
...
#    if 'tt' in ch: gr_tot.GetYaxis().SetRangeUser(-5,15 )
#    if 'all' in ch: gr_tot.GetYaxis().SetRangeUser(-2,5 )
...
#    gr_bins.Draw( 'Pesame' )
    ...
    c1.SaveAs('muValue_multidim_{}_{}.pdf'.format(inputDir, ch))
