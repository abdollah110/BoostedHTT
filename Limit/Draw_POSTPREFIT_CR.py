...
#    ZJ.Rebin(RB_)
    ...
    DYS=file.Get(categoriy).Get("DYJets")
    DYS.Rebin(RB_)
...
#    legende.AddEntry(QCD,"QCD multijet","f")
    legende.AddEntry(DYS,"DY+jets ","f")
...
#    c.Modified()
    h1.GetYaxis().SetRangeUser(.9,1.1)
#    c.Modified()
    c.SaveAs("_Finalplot_DR_"+prefix+categoriy+Status+"_CMB_"+Channel+".pdf")
...
FileNamesInfo=[
#               [InputRootfile,"CR_postfit","m_{vis} [GeV]","PostFit (Pass)","mt"],
               [InputRootfile,"ch1_postfit","m_{vis} [GeV]","PostFit (CR)","mt"],
#               [InputRootfile,"CR_prefit","m_{vis} [GeV]","PreFit (Pass)","mt"],
               [InputRootfile,"ch1_prefit","m_{vis} [GeV]","PreFit (CR)","mt"],
