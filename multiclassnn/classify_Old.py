...
def fillFile(ifile, channel, args, vbf_pred, boost_pred):
  ...
  print 'Starting process for file: {}'.format(fname), '  channel is ', channel, ifile,args.treename
...
#  boost_pred.make_prediction(fname, channel)
  ...
  print 'itree name', itree.GetName()
...
#  disc_branch1 = ntree.Branch('NN_disc_ZTT', branch_var1, 'NN_disc_ZTT/F')
...
#  disc_branch_boost = ntree.Branch('NN_disc_boost', branch_var_boost, 'NN_disc_boost/F')
  ...
  evt_index = 0
  for _ in itree:
    if evt_index % 1000 == 0 and evt_index > 0:
