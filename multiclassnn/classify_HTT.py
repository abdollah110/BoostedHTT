...
def fillFile(ifile, channel, args, vbf_pred):
  ...
  vbf_pred.make_prediction(fname, channel, args.Transform, args.nominal_input)
...
#    vbf_pred = Predictor(args.input_vbf, args.model_vbf, keep_vbf)
    vbf_pred = Predictor(args.input_vbf, args.model_vbf)
    processes = [Process(target=fillFile, args=(ifile, 'em', args, vbf_pred)) for ifile in file_names]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model-vbf', '-m', action='store', dest='model_vbf', default=None, help='name of model to use')
    parser.add_argument('--input-vbf', '-i', action='store', dest='input_vbf', default=None, help='name of input dataset')
