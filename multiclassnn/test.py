...
selection_vars = ['is_signal','OS','Pass']

# Variables that could be used as NN input. These should be normalized
scaled_vars = ['evtwt','muPt','taupt','lepIso','tmass', 'ht','Met','LeadJetPt', 'dR_mu_tau']
...
def main(args):
    ...
    names, leptons, isSignal,isZTT,weight_df, index = np.array([]), np.array([]), np.array([]), np.array([]), np.array([]), np.array([])
