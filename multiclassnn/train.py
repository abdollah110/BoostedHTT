...
#    print 'training_processes = ', training_processes
...
#    model.add(Dense(1, activation='softmax', kernel_initializer='glorot_normal')) # change sigmoid to softmax for multiNN
#    model.summary()
...
#        (training_processes['muPt'] > 50)
#        ]
#    print 'vbf_processes = ', vbf_processes
...
#
#
#    lowest = max(len(sig_df), len(bkg_df), len(bkg_df2))
#    sig_df.loc[:, 'evtwt'] = sig_df['evtwt'].apply(lambda x: x*lowest/len(sig_df)).values
#    bkg_df.loc[:, 'evtwt'] = bkg_df['evtwt'].apply(lambda x: x*lowest/len(bkg_df)).values
#
#    if '2016' in args.year:
#        bkg_df2.loc[:, 'evtwt'] = bkg_df2['evtwt'].apply(lambda x: 0.706*lowest/len(bkg_df2)).values # scale to get correct TT/ZTT
#    elif '2017' in args.year:
#        bkg_df2.loc[:, 'evtwt'] = bkg_df2['evtwt'].apply(lambda x: 0.605*lowest/len(bkg_df2)).values # scale to get correct TT/ZTT
#    elif '2018' in args.year:
#        bkg_df2.loc[:, 'evtwt'] = bkg_df2['evtwt'].apply(lambda x: 0.658*lowest/len(bkg_df2)).values # scale to get correct TT/ZTT
#    else:
...
#    training_labels = training_labels[:, 0:3]
#    training_weights = training_labels[:, 3]
