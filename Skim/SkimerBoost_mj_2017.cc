...
//              else if (name.find("HLT_PFHT500_PFMET100_PFMHT100_IDTight_v")                         != string::npos) bitJet = 39;
    ...
    MHT= sqrt(MHT_x*MHT_x + MHT_y*MHT_y);
        ...
        for (int ijet=0; ijet < nAK8Jet ; ijet ++){
            ...
            //Filling 1D eff plot
            if (AK8JetPt->at(ijet) > 500  && AK8JetSoftDropMass->at(ijet) > 0 && fabs(AK8JetEta->at(ijet)) < 2.5)
                plotFill("ht_trgEff_1D_Before",AK8JetSoftDropMass->at(ijet),30,0,300);
            if (AK8JetPt->at(ijet) > 500  && AK8JetSoftDropMass->at(ijet) > 0 && fabs(AK8JetEta->at(ijet)) < 2.5 && HLT_AK8PFJet400_TrimMass30)
        ...
        //Filling 1D eff plot
        if (PFHT > 600 )
            plotFill("met_trgEff_1D_Before",PFMET+MHT,100,0,2000);
        if (PFHT > 600  && HLT_PFHT500_PFMET100_PFMHT100_IDTight)
            plotFill("met_trgEff_1D_After",PFMET+MHT,100,0,2000);
